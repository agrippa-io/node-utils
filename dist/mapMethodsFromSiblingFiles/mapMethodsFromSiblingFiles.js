...
const getFilenames_1 = require("../getFilenames");
const DEFAULT_OPTIONS = {
    isDefaultModule: true,
    asLastNode: true,
};
exports.default = async (path, options = DEFAULT_OPTIONS) => {
    const _options = {
        ...DEFAULT_OPTIONS,
        ...options,
    };
    const { isDefaultModule, asLastNode } = _options;
    const filenames = (0, getFilenames_1.getFilenames)(path, { excludeExtension: true });
    return filenames.reduce(async (acc, filename) => {
        const methodName = asLastNode ? filename.split('.').pop() ?? filename : filename;
        if (methodName !== 'index') {
